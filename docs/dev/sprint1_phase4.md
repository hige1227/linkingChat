> **状态：✅ 核心验证通过** | 完成日期：2026-02-14 | 全链路手动验证通过（64ms），详见 [sprint1_implement_mark.md](./sprint1_implement_mark.md)
...
- [x] 确认 `.env` 配置正确（数据库连接、JWT 密钥路径、Redis 地址）
- [x] 确认已执行 `pnpm build`，所有 shared packages 编译成功
...
| 9 | 断开桌面端网络（拔网线或关闭 Wi-Fi），等待心跳超时 | Server 检测到桌面端离线，移动端设备列表状态更新为"离线" | ⏭ 延迟 |
| 10 | 恢复桌面端网络 | 桌面端自动重连，状态恢复为"已连接"，移动端设备列表同步更新 | ⏭ 延迟 |
| 11 | 桌面端离线状态下，从移动端发送命令 | 移动端收到错误提示："目标设备离线" | ⏭ 延迟 |
...
|-------|---------|------|---------|---------|--------|
| B001 | Phase 3 flutter_secure_storage | SubtleCrypto OperationError on Web | P0 | ✅ 已修复 | Claude |
| B002 | Phase 3 页面刷新 | 401 无限循环，GoRouter 未做 auth 重定向 | P0 | ✅ 已修复 | Claude |
| B003 | Phase 3 socket_io_client | Web 端 setTransports(['websocket']) 导致收不到事件 | P0 | ✅ 已修复 | Claude |
| B004 | Phase 3 命令匹配 | commandId 不匹配（客户端 vs Prisma cuid） | P0 | ✅ 已修复 | Claude |
| B005 | Phase 4 危险命令 | emitWithAck ACK 回调不触发 StateNotifier 重建 | P1 | ✅ 已修复 | Claude |
...
- [x] 所有 P0 Bug 已修复并验证
- [x] 所有 P1 Bug 已修复并验证
- [x] 补充测试过程中发现的缺失错误处理（try-catch、WS error handler 等）
- [x] 确认日志输出清晰，关键节点有 log（连接、断开、命令收发、执行结果）
- [x] 确认没有明显内存泄漏（长时间运行桌面端 + 反复发命令不应导致内存持续增长）
