...
4. AI 模块（Q5 确认：三个模式全做）
   ...
   b. The Whisper（耳语建议）[P1]
      - **MVP**：用户 `@ai` 主动触发 → LLM 生成 1 个最优回复（灰体预填入输入框）+ `···` 展开 2 个备选
      - **不做**：自动推送建议气泡（已否决"输入框上方 3 个 Chips"方案，体验太 generic）
      - 使用 DeepSeek（低延迟）做建议生成
      - 数据表：ai_suggestions (type='whisper')
      - **v2+ 计划**：灰体补全（Ghost Text），触发条件：输入框获焦 + ≥2秒未输入 + 无字符，基于本地小模型意图预测（肯定/否定/提问），高置信度时才显示
...
## 八、设计讨论决策记录 (2026-02-13)

> 基于团队设计讨论（battle session），对以下方向达成共识并记录。

### 8.1 The Whisper 交互重设计

**已否决方案**：输入框上方自动悬浮 3 个建议气泡（Chips）。理由：体验太 generic（类似 Google Smart Reply），容易退化为"好的/谢谢/收到"等废话。

**MVP 方案**：

| 项目 | 决策 |
|------|------|
| 触发方式 | 用户 `@ai` **主动触发**（非自动推送） |
| 输出形式 | 1 个最优回复预填入输入框 + `···` 展开 2 个备选 |
| 生成模型 | DeepSeek（低延迟） |
| 延迟要求 | <2 秒（用户主动触发有等待预期，不需要 <800ms） |

**v2+ 演进 — 灰体补全 (Ghost Text)**：

| 项目 | 决策 |
|------|------|
| 触发条件 | 输入框获焦 + ≥2秒未输入 + 无字符输入 |
| 意图预测 | 本地小模型判断用户意图（肯定/否定/提问），高置信度时才显示 |
| 交互 | Tab 采纳，Esc 忽略 |
| 依赖 | 本地小模型部署能力（聊天补全 ≠ 代码补全，自然语言开放度高，需专用模型） |

**为什么不能照搬 VS Code Copilot 模式**：代码补全靠语法规则缩小预测空间，聊天回复需要理解社交语境和情绪，错误成本高（误发给老板 vs 按 Esc 取消代码补全）。

### 8.2 移动端 UI 方向

| 项目 | 决策 |
|------|------|
| 设计参考 | 微信 / WhatsApp 风格，less is more |
| Bot 入口 | **固定置顶**的系统级联系人（类似微信"文件传输助手"），不可删除 |
| Bot 形态 | Bot 即好友，在聊天列表中以联系人形式存在，每个 bot 有独立聊天框 |

### 8.3 Supervisor Bot

**定位**：通知汇总中心 + 智能前台，不是唯一入口。

| 能力 | 说明 |
|------|------|
| 调度 | 用户不确定找谁时，Supervisor 引导到对应 bot |
| 通知汇总 | 所有 bot 的关键事件以通知卡片形式出现在 Supervisor 聊天流中 |
| 兜底 | 群聊中 @ai = 调用 Supervisor |

**Supervisor 聊天框 UI：聊天流 + 通知卡片**

不使用 tab 或独立通知面板（违反 less is more）。通知以特殊 `MessageType.BOT_NOTIFICATION` 渲染为卡片样式，穿插在正常对话流中。用户可以就着通知继续对话。

```
Supervisor 聊天框:

  ┌────────────────────────────────────────┐
  │ 📋 Coding Bot 完成了测试任务 ✅        │  ← BOT_NOTIFICATION 卡片
  │ 3 分钟前 · 耗时 45s · [查看结果]       │
  └────────────────────────────────────────┘

  用户: 社媒那边有什么热点吗？
  Supervisor: 你可以直接问社媒 Bot，要我帮你转达吗？

  ┌────────────────────────────────────────┐
  │ 🔥 社媒 Bot 发现 3 条热点             │  ← BOT_NOTIFICATION 卡片
  │ 1 分钟前 · [查看详情] [让社媒Bot写推文] │
  └────────────────────────────────────────┘
```

### 8.4 多 Bot 架构

**核心结论**：多 Bot 框架 MVP 就做进去（成本低），但只实现一种能力。

**MVP Bot 初始化**：用户注册后系统自动创建：
1. **Supervisor Bot**（置顶，不可删除）— 欢迎消息 + 引导
2. **Coding Bot**（置顶，可配置）— 欢迎消息 + 设备连接引导

理由：MVP 第一里程碑是"手机→电脑执行"，如果用户还要手动创建 bot 才能用，首次体验就断了。从第一天就让用户看到两个 bot，建立"不同 bot 做不同事"的心智模型。

| 阶段 | 做什么 |
|------|--------|
| MVP | 多 bot 框架（bot CRUD、bot 列表 UI、消息路由到对应 agent config）+ **仅远程执行能力** |
| v1.x | 按需加 bot 类型，每加一种 = 实现对应工具链（社媒运营、数据分析等） |
| v2.0 | 开放自定义 bot，用户可创建多个专用 agent |

**架构分层**：

```
容易的部分（OpenClaw 已支持）：
├── Agent 配置层：不同的 system prompt + 不同的 LLM 路由 + 不同的工具集
├── Bot 列表 UI：名称、头像、描述，CRUD 操作
└── 聊天路由：消息发到哪个 bot → 路由到对应 agent config

需要逐步实现的部分：
└── 每个 bot 类型的工具链
    ├── 远程执行 Bot：shell 执行、文件读写、git 操作 → OpenClaw 已有（MVP）
    ├── 社媒运营 Bot：Twitter API、Instagram API、内容排期 → v1.x
    └── 其他类型：每多一种 = 一套新的工具集成
```

### 8.5 Bot 间通信

**核心规则：所有 bot 均可互相通信，但需要标注触发来源。**

| 行为 | 是否允许 | 条件 |
|------|---------|------|
| Bot 在自己聊天框通知用户 | ✅ 始终允许 | — |
| Supervisor 汇总通知 | ✅ 始终允许 | 所有 bot 的关键事件汇总到 Supervisor |
| Bot A → Bot B 通信 | ✅ 允许 | OpenClaw 层面直接支持 |
| Bot B 因 Bot A 触发而通知用户 | ✅ 允许 | **必须标注触发来源**（如 "[来自 Coding Bot 的协作]"） |
| Bot 自主执行动作 | ❌ 仍需确认 | Draft & Verify 不变 |

**触发来源标注示例**：
```
社媒 Bot 聊天框:
  [来自 Coding Bot 的协作] Coding Bot 完成了数据爬取，需要我帮你写推文吗？
  [查看数据] [写推文]
```

### 8.6 @ai 在群聊中的行为

**方案：Bot 可被拉入群聊（Telegram 模式），@具体Bot 精准调用，@ai = Supervisor 兜底**
...
为什么不做 Supervisor 自动路由：多一层路由 = 多一层延迟和出错点。用户在群里知道想找谁，"魔法路由"路由错了比没路由更糟。

### 8.7 后端技术选型：NestJS 确认，Rust 否决

| 项目 | 决策 |
|------|------|
| 后端框架 | **NestJS (Node.js / TypeScript)** ✅ 已确认 |
| Rust | ❌ 已否决 |

**否决 Rust 的理由**：

1. 团队已确认 TypeScript everywhere，前后端共享类型/协议是核心效率优势
2. 2-3 人团队，Rust 学习曲线直接拖垮 MVP 进度
3. 性能瓶颈在 LLM API 调用延迟（几百 ms ~ 几秒），NestJS 处理 WebSocket 消息转发是微秒级
4. NestJS 单实例轻松扛 5000-10000 WebSocket 并发，加水平扩展（多实例 + Redis）可达 10 万级

**扩展性靠架构而非语言**：

| 真正的瓶颈 | 解法 |
|-----------|------|
| WebSocket 连接数 | Nginx/HAProxy 负载均衡 + 多实例 |
| 消息持久化 | PostgreSQL 分表/分库 |
| LLM 调用延迟 | 异步队列 + 流式响应 + 缓存 |
| 状态同步 | Redis pub/sub 跨实例同步 |

**正确的演进路径**：NestJS 快速验证产品 → 产品成功后用数据找到真正瓶颈 → 针对性优化（可能是 Rust 重写某个微服务，也可能只是加缓存）。Discord 也是先用 Python 验证产品，几百万并发时才引入 Rust 重写热路径。
